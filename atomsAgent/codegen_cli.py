#!/usr/bin/env python3
"""Standalone schema synchronization utility with optimized processing."""
...
async def check_database_connection(db_url: str) -> bool:
    ...
    try:
        # For Supabase local, we'll check if port is open
...
async def parallel_schema_processing(
    ...
    max_workers: int = 4,
    db_url: str = "postgresql://localhost:54322/supabase",
) -> dict[str, Any]:
    ...
    with Progress(
        ...
        # Submit all schema processing tasks using simple approach
        tasks = []
        for schema_file in schema_files:
            task = asyncio.create_task(
                process_single_schema(schema_file, output_dir, progress, db_url)
            )
            tasks.append((task, schema_file))

        # Collect results as they complete
        for task, schema_file in tasks:
            try:
                result = await task
                results[str(schema_file)] = result
            except Exception as e:
                console.print(f"[red]Error processing {schema_file}: {e}[/red]")
                results[str(schema_file)] = {"error": str(e)}
...
async def process_single_schema(
    ...
    try:
        ...
        result = await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: run_with_progress(cmd, f"Processing {schema_file.name}", progress, task_id),
        )
...
def sync(
    ...
    if schema_path is None:
        ...
        schema_path = script_dir.parent / "database"

    if output_dir is None:
        script_dir = pathlib.Path(__file__).parent
        output_dir = script_dir / "src" / "atomsAgent" / "db"
